...
pairs_restriction_fp <- args[6]
optional_args <- if (length(args) == 7) args[7] else NULL
...
# obtain the (response, grna) pairs to analyze -- downsample (if necessary) and restrict to pairs_restriction
...
if (pairs_restriction_fp != "null") {
  pairs_restriction_df <- readRDS(pairs_restriction_fp)
  response_ids <- pairs_restriction_df |>
    dplyr::filter(undercover_grna == undercover_ntc_name_in,
...
if (length(response_ids) == 0) {
  result_df <- as.data.frame(matrix(nrow = 0, ncol = 3))
  colnames(result_df) <- c("grna_group", "p_value", "response_id")
} else {
  response_grna_group_pairs <- data.frame(response_id = response_ids, grna_group = "undercover")
