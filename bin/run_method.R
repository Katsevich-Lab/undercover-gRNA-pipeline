...
out <- result_df |>
  dplyr::mutate(test_ntc = gRNA_group, gRNA_group = NULL, dataset = dataset_name, method = method_name, ram_req = ram_req) |>
  dplyr::mutate_at(.vars = c("response_id", "test_ntc", "dataset", "method", "ram_req"), .funs = factor)
